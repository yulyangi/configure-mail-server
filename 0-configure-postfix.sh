...
dbname='mail'
host='127.0.0.1'
query_domain="SELECT domain FROM domains WHERE domain='%s'"
query_forwardings="SELECT destination FROM forwardings WHERE source='%s'"
query_mailboxes="SELECT CONCAT(SUBSTRING_INDEX(email,'@',-1),'/',SUBSTRING_INDEX(email,'@',1),'/') FROM users WHERE email='%s'"
